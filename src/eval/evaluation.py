...
from src.wiqa_wrapper import WIQAQuesType, download_from_url_if_not_in_cache, Jsonl, WIQAExplanation, \
    WIQAExplanationType
...
class InputRequiredByFineEval:
    ...
    @staticmethod
    def from_(prediction_on_this_example: WIQAExplanation,
              json_from_question: Dict,
              expl_type: WIQAExplanationType):
        # get expected answer and whether predicted and expected match.
        expected = WIQAExplanation.instantiate_from(json_from_question)

        metrics = {}
        metrics[FineEvalMetrics.EQDIR] = expected.de == prediction_on_this_example.de
        if expl_type == WIQAExplanationType.PARA_SENT_EXPL:
            metrics[FineEvalMetrics.XDIR] = expected.di == prediction_on_this_example.di
            metrics[FineEvalMetrics.XSENTID] = expected.i == prediction_on_this_example.i
            metrics[FineEvalMetrics.YSENTID] = expected.j == prediction_on_this_example.j
        return InputRequiredByFineEval(graph_id=json_from_question["metadata"]["graph_id"],
                                       ques_type=WIQAQuesType.from_str(json_from_question["metadata"]["question_type"]),
                                       path_arr=[],
                                       metrics=metrics)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--group_by',
                        ...
                        dest='group_by',
                        required=True,
                        help='questype|pathlen')

    parser.add_argument('--from_model',
                        action='store',
                        dest='from_model',
                        required=True,
                        help='your_model_name|no_explanation_model')
    ...
    with open(args.pred_path) as infile:
        for line in infile:
            j = json.loads(line)
            if args.from_model == "no_explanation_model":
                entry = InputRequiredByFineEval.from_(expl_type=WIQAExplanationType.NO_EXPL,
                                                      json_from_question=j["orig_question"],
                                                      prediction_on_this_example=WIQAExplanation.instantiate_from(j)
                                                      )
            else:
                raise NotImplementedError(f"fine_eval script does not support model: {args.from_model}, "
                                          f"implement your own here in evaluation.py")
